...
class Runner(AbstractEnvRunner):
    ...
    def __init__(self, *, env, model, nsteps, gamma, lam):
        ...
        self.terminals = [[False] for _ in range(self.nenv)]

    def run(self):
        # Here, we init the lists that will contain the mb of experiences
        mb_obs, mb_rewards, mb_actions, mb_values, mb_dones, mb_neglogpacs, mb_terminals = [],[],[],[],[],[],[]
        ...
        for t in reversed(range(self.nsteps)):
            if t == self.nsteps - 1:
                nextnonnew = 1.0 - self.dones # crash/episode end
                nextnonterminal = 1.0 - self.terminals #crash
                nextvalues = last_values
            else:
                nextnonnew = 1.0 - mb_dones[t+1]
