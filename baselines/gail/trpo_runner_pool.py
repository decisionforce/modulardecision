...
class Runner(object):
    def __init__(self, env, model, nsteps, gamma, lam, length=800, rew_type=False, model_output='DM'):
        ...
        self.gamma = gamma
        self.lam = lam
        self.nsteps = nsteps * int(self.nenv)
    ...
    def run(self):
        ...
        if self.model_output == 'DM':
            ac, v_init, _ = self.model.step_decision(self.obs)
            _, _, _, info = self.env.step([1., 0., 0.])
        else:
        ...
        total_steps = [0 for i in range(self.nenv)]
        mb_obs = [np.array([self.init_obs[0] for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_actions = [np.array([self.env.action_space.sample() for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_values = [np.array([0. for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_news = [np.array([False for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_terminals = [np.array([False for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_controls = [np.array([[0.0, 0.0] for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_true_rewards = [np.array([0. for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_rewards = [np.array([0. for i in range(self.nenv)]) for i in range(self.nsteps)]
        mb_infos = [[info[0] for i in range(self.nenv)] for i in range(self.nsteps)]
        e = 0
        mb_obs[0][:] = self.obs.copy()
        mb_news[0][:] = self.news
        mb_terminals[0][:] = self.terminals
        need_revise = [False for i in range(self.nenv)]
        revise_idx = [None for i in range(self.nenv)]
        while not np.sum(np.array(total_steps))==self.nsteps:
            nb_actions = []
            true_rewards = [0. for i in range(self.nenv)] 
            infos = [None for i in range(self.nenv)] 
            ...
            self.env.env_now = e
            self.news = np.array(self.news)
            ob, true_reward, new, info = self.env.step(actions)
            
            if new[0]:
                #ob = [self.init_obs[e]]
                need_revise[e] = True
                revise_idx[e] = total_steps[e]
                reset_e_now = e
            self.obs[e], true_rewards[e], self.news[e], infos[e] = ob[0], true_reward[0], new[0], info[0]
            # ------------------------------------------------------------------
            step = total_steps[e]
            mb_obs[step][e] = self.obs[e]
            mb_actions[step][e] = np.asarray(actions[e], dtype=np.float32)
            mb_values[step][e] = np.asarray(values[e], dtype=np.float32)
            mb_news[step][e] = self.news[e]
            mb_terminals[step][e] = self.terminals[e]
            
            maybeepinfo = info[0].get('episode')
            if maybeepinfo: epinfos.append(maybeepinfo)
            control = infos[e].get('control')
            controls = [control for i in range(self.nenv)]
            mb_controls[step][e] = np.asarray(control, dtype=np.float32)
            # ------------------------------------------------------------------
            if self.model_output == 'DM':
                rewards = self.model.fake_rew(self.obs, controls) 
            eprew_tmp += rewards
            self.terminals = np.array([infos[e]['terminal'] for info in infos])
            
            ## add episode start id
            if self.news[e]:
                v_ep[e].append(values[e])
                eprews_tmp[e].append(eprew_tmp[e])
                eprew_tmp[e] = 0.

            mb_true_rewards[step][e] = true_rewards[e]
            mb_rewards[step][e] = rewards[e]
            mb_infos[step][e] = infos[e]
            total_steps[e] += 1

            if np.any(np.array(need_revise)):
                for reset_e in range(len(need_revise)):
                    if need_revise[reset_e] and not self.env.venv.venv.is_reset(reset_e):
                        # revise observation
                        ob = self.env.venv.venv.get_reset_ob(reset_e)
                        mb_obs[revise_idx[reset_e]][reset_e] = ob
                        self.init_obs[reset_e] = ob
                        need_revise[reset_e] = False

            ## switch to a waiting simulator
            if self.news[e]:
                valid_env = [i for i in range(self.nenv) if not self.env.venv.venv.is_reset(i)]
                if len(valid_env) >= 1:
                    e = random.choice(valid_env)
                else:
                    pass
                self.obs[e] = self.init_obs[e]

        if np.any(np.array(need_revise)):
            for reset_e in range(len(need_revise)):
                if need_revise[reset_e]: 
                    while True:
                        if not self.env.venv.venv.is_reset(reset_e):
                            ob = self.env.venv.venv.get_reset_ob(reset_e)
                            mb_obs[revise_idx[reset_e]][reset_e] = ob
                            break
        ...
        output = [*map(stack, (mb_obs, mb_rewards, mb_returns, mb_news, mb_true_rewards, mb_actions, mb_values, mb_advs, mb_infos, mb_controls))]
        obs, rews, returns, news, true_rews, acs, vpreds, advs = output[:-2]
        ctrls = output[-1]
