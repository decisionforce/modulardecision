...
def argsparser():
    ...
    parser.add_argument('--scenario', action='store_true', help='excute mode for the decision model')
    ...
    def __init__(self, args, actor_id):
        ...
        def make_env(host, port, args):
            ...
            world = World(carla_world, args.sync, args.sigmas, camera=args.render, A_skip=args.A_skip, mode=args.mode, feature=args.feature, 
                          ...
                          scenario_name=args.scenario_name, client=client, host=host, port=port, scenario=args.scenario)
...
def main(args):
    ...
    actors = [Actor(args, i) for i in range(args.actor_nums)]
    args = actors[0].get_args()

    learner = Learner(args)

    for i in range(args.max_iters):
        logger.log("********** Iteration %i ************" % i)
        batches = [actor.get_batch() for actor in actors]
        batches = batches_filter(batches)
        params = learner.learn(batches)
        status = [actor.update_params(params) for actor in actors]
    return
