...
def argsparser():
    ...
    parser.add_argument('--actor_nums', default=1, type=int, choices=[1, 2, 3, 4, 8, 16, 32, 64], help='Actor numbers')
    ...
    def __init__(self, args, actor_id):
        ...
        def make_env(host, port, args):
            ...
            world = World(carla_world, args.sync, args.sigmas, camera=args.render, A_skip=args.A_skip, mode=args.mode, feature=args.feature, 
                          ...
                          scenario_name=args.scenario_name, client=client, host=host, port=port)            
...
def main(args):
    ...
    if len(host) % int(args.actor_nums) == 0:
        ...
        assert args.actor_nums == 3
        print(args.host)
        assert len(host) == 4
        hosts_pre = [[ip] for ip in host[:-2]]
        hosts_last = []+[host[-2]]+[host[-1]]
        hosts_pre.append(hosts_last)
        hosts = hosts_pre
        ports_pre = [[port] for port in port[:-2]]
        ports_last = []+[port[-2]]+[port[-1]]
        ports_pre.append(ports_last)
        ports = ports_pre
        print(hosts, ports)
        args.hosts = hosts
        args.ports = ports
