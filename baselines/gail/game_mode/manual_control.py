...
# Allows controlling a vehicle with a keyboard or a joystick. For a simpler and more
...
class JoystickControl(object):
    ...
    def update_button(self):
        ...
        self.throttle_brake_axises = self.joystick.get_axis(1)
        self.reverse_button = self.joystick.get_button(1)
        self.reset_button = self.joystick.get_button(5)
        self.camera_control_button = self.joystick.get_button(4)
        self.weather_control_button = self.joystick.get_axis(5)
        self.sensor_control_button = self.joystick.get_button(6)
        self.recording_button = self.joystick.get_button(7)
    ...
    def _parse_vehicle_keys(self, milliseconds):
        if self.throttle_brake_axises<0:
            throttle = -self.throttle_brake_axises 
            brake = 0
        else:
            throttle = 0
            brake = 1
        throttle_brake = np.clip(throttle+-1*brake, -1, 1)
        ...
        throttle_brake = np.clip(time_throttle+self.world.throttle_brake, -1, 1)
        if throttle_brake < 0:
            throttle = 0
            brake = 1
        else:
...
class DrivingForceControl(object):
    ...
    def update_button(self):
        self.steering_axises = self.joystick.get_axis(0)
        self.throttle_axises = self.joystick.get_axis(1)
        self.brake_axises = self.joystick.get_axis(2)
        # self.reverse_button = self.joystick.get_button(1)
        self.reset_button = self.joystick.get_button(5)
        self.camera_control_button = self.joystick.get_button(4)
