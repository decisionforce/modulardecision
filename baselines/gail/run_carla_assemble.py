...
class CarlaUE4Monitor(object):
    ...
    def _init_server(self):
        carla_cmd = 'DISPLAY=:8 ' + os.path.join(self.args.carla_path+'/CarlaUE4.sh')
        carla_arg = ' -world-port=2000 --carla-server -fps 20 -carla-settings=./CarlaSettings.ini -windowed -ResX=100 -ResY=100 -opengl3'
        carla_cmd += carla_arg
        self.server_process = subprocess.Popen(carla_cmd,shell=True,preexec_fn=os.setsid)
        time.sleep(150) ##wait for Carla Server to launch ##
        logger.info('service group id:%s'%os.getpgid(self.server_process.pid))
        ...
        logger.info('############################## Monitor succesfully opened carla with command #######')
